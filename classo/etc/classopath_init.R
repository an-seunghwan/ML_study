...
constraints = list(Aeq2 %*% target == Aeq3 %*% target,
                   ...
                   Aeq2 %*% target >= Aeq4 - Aeq5 %*% target,
                   Aeq1 %*% target < rho_min)
objective = Maximize(Aeq1 %*% target)
problem = Problem(objective, constraints)
result = solve(problem)
...
target = result$getValue(target)
rho2 = target[1, ]
z2 = target[2:(1+p), ,drop=F]
lambda2 = target[(p+2):nrow(target), ,drop=F]
rho2; z2; lambda2
# 범위를 벗어나는 predictor
idx1 = which((-t(X) %*% y + rho2 * one_p) < t(Aeq) %*% lambda2)
idx2 = which((-t(X) %*% y - rho2 * one_p) > t(Aeq) %*% lambda2)
activeset = c(idx1, idx2)
sort(activeset) # 최종 active set
